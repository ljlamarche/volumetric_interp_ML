# NEED TO ADD THE ALTITUDE LIMIT TO THIS FUNCTION
...
def read_AC_datafile(filenames, start_time, end_time, chi2lim=(0.1, 10), Alt_lim=None):
    """
    ...
    for filename in filenames:
        ...
        if Alt_lim is not None:
            altitude_mask = altitude <= Alt_lim
            altitude = altitude[altitude_mask]
            latitude = latitude[altitude_mask]
            longitude = longitude[altitude_mask]
            value = value[:, altitude_mask]
            error = error[:, altitude_mask]
