...
def volumetric_nn(df, resolution=(10, 10, 10), cbar_lim=None, real_dist=False, density_range=(1e10, 1e12),
    ...
    # Finding the number of epochs and neurons that produce the highest R^2 coefficient.
    epoch, neurons_range, score = optimizer(x, y, weights)

    # Defining the base of the model.
    network = tf.keras.models.Sequential()
    network.add(tf.keras.layers.Dense(neurons_range[0], input_dim=3, activation='tanh'))
    # Adding all of the layers with the neurons in neurons_range.
    for neurons in neurons_range[1:]:
        network.add(tf.keras.layers.Dense(neurons, activation='tanh'))
    # Adding the last layer of the network.
    ...
    # Early stopping to prevent overtraining; stopping training if the loss doesn't improve after 12 epochs.
    early_stopping = tf.keras.callbacks.EarlyStopping(monitor='loss',mode='auto',verbose=0,patience=12)
    
    # Train the network.
    # Comment out this line if you already have a weights.h5 file and you want to skip the training process.
    network.fit(x, y, epochs=epoch, sample_weight=weights, callbacks=[StopAtLossValue(), checkpoint, early_stopping])
    ...
    print(epoch, neurons_range[0], len(y), len(weights))


def optimizer(x, y, weights):
    # The range of epochs and neurons to check.
    epoch_range = range(100, 301, 50)
    ...
    def find_parameters():
        ...
        for epoch in epoch_range:
            for i,neurons in enumerate(neurons_range[:-3]):
                pos = i
                nn = tf.keras.models.Sequential()
                # Adding the first layer of the neural network.
                nn.add(tf.keras.layers.Dense(neurons_range[pos], input_dim=3, activation='tanh'))
                pos += 1
                # Adding the rest of the layers of the neural network.
                while pos < len(neurons_range):
                    nn.add(tf.keras.layers.Dense(neurons_range[pos], activation='tanh'))
                    pos += 1
                # Adding the last layer of the neural network.
                nn.add(tf.keras.layers.Dense(1, activation='sigmoid'))

                # Compile the network: Define the optimizer and the loss function. See the following link for reference:
                # https://medium.com/data-science-group-iitr/loss-functions-and-optimization-algorithms-demystified-bb92daff331c
                nn.compile(optimizer=tf.keras.optimizers.Adam(), loss=tf.keras.losses.MeanAbsoluteError())

                # Define a checkpoint. This allows the network to save the "best weights" to an h5 file.
                checkpoint = tf.keras.callbacks.ModelCheckpoint('weights.keras', verbose=1, monitor='loss', save_best_only=True, mode='auto')
                # Train the network.
                # Comment out this line if you already have a weights.h5 file and you want to skip the training process.
                nn.fit(x, y, epochs=epoch, sample_weight = weights, callbacks=[StopAtLossValue(), checkpoint])
                # Compute and display r2 coefficient.
                y_hat = nn.predict(x)
                y_pred = list(y_hat.T[0])
                y_true = list(y)
                new_score = metrics.r2_score(y_true, y_pred)
                if score < new_score:
                    best_epoch = epoch
    ...
    return epoch, neurons_range, score
