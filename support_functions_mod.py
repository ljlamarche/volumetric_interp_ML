...
def volumetric_nn(df, start, end, resolution=(10, 10, 10), cbar_lim=None, real_dist=False, density_range=(1e10, 1e12),
    ...
    # Training the Artifical Neural Network 10 times (can be changed).
    for i in range(10):
        ####################
        ## NEURAL NETWORK ##
        ####################
        ...
        # Early stopping to prevent overtraining; stopping training if the loss doesn't improve after 10 epochs.
        early_stopping = tf.keras.callbacks.EarlyStopping(monitor='loss',mode='auto',verbose=0,patience=20)
        # Train the network.
    ...
    sum = 0  # Sum of all of the predictions.
    ...
    # Creating a copy of the original data frame to filter out "out-of-bounds" predictions.
    df_new = df.copy()

    # Normalizing the latitude and longitude data.
    ...
    def update(alt_value, save=False):
        ...
        l, r, b, u = np.min(pts['Longitude']), np.max(pts['Longitude']), np.min(pts['Latitude']), np.max(pts['Latitude'])
