...
def volumetric_nn_2(df, start, end, resolution=(10, 10, 10), cbar_lim=None, real_dist=False, density_range=(1e10, 1e12),
    ...
    for i in range(15):
        ...
        checkpoint = tf.keras.callbacks.ModelCheckpoint('weights.keras', verbose=1, monitor='loss', save_best_only=True,
                                                        mode='auto')
    ...
    if fig3D_2:
        ...
        if real_dist:
            ...
            plt.ylabel('Latitude (°)')
            plt.suptitle(f'Alternating Code (UTC): {start} - {end:%H:%M:%S}')
