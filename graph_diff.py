...
def get_data():
    ...
    datas = [["chapman_data_025.h5", "chapman_data_ac.h5"], ["circular_data_025.h5", "circular_data_ac.h5"], ["gradient_data_025.h5", "gradient_data_ac.h5"], ["tubular_data_025.h5", "tubular_data_ac.h5"], ["gradient_circular_data_ac.h5", "gradient_circular_data_025.h5"], ["chapman_circular_data_ac.h5", "chapman_circular_data_025.h5"], ["chapman_gradient_data_ac.h5", "chapman_gradient_data_025.h5"], ["wave2_data_ac.h5", "wave2_data_025.h5"], ["two_circles_data_ac.h5", "two_circles_data_025.h5"]]
    config_files = ['chapman_config.yaml', 'circular_config.yaml', 'gradient_config.yaml', 'tubular_config.yaml', "gradient_circular_config.yaml", "chapman_circular_config.yaml", "chapman_gradient_config.yaml", "wave2_config.yaml", "two_circles_config.yaml"]

    # n: Determines which datafile is tested.   "chapman_data_ac.h5"
    n = 1

    # Inputs: Directory to data file, directory to configuration file, start time, and end time.
    data_file = datas[n]
    ...
    # Finding multiple mean average errors (MAEs) to find the mean and standard deviation. 
    mae_list = []
    for i in range(3):
        df, stations = create_all_data(start, end, data_file, 0, config_file)
        mae_list.append(np.mean(abs(np.array(df['Value']))))

    # Calculating the mean MAE and standard deviation.
    ...
    # Creating the reference data to find the limits for plotting.
    ...
    # Filtering data so that only points within the "cone" region are shown.
    df_filtered = filter_data(df, ref_data, 30)
